...
export const useTxs = makeUseWebSocket(
    ...
    [] as Tx[],
    txParser,
    ( { x: { hash, time, inputs, out } } ) => txs => [ ...txs.filter( tx => tx.time > time && tx.hash !== hash ), { io: [ ...inputs.map( _ => ( { address: _.prev_out.addr ?? "?????", value: - _.prev_out.value } ) ), ...out.map( _ => ( { address: _.addr ?? "?????", value: _.value } ) ) ], hash, time }, ...txs.filter( tx => tx.time <= time && tx.hash !== hash ) ].slice( 0, 100 ),
